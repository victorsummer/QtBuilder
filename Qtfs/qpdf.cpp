    ...
    for (int step = from; step < to; ++step) {
        if (reflect && step % 2) {
            for (int i = stops.size() - 1; i > 0; --i) {
                QGradientBound b;
                b.start = step + 1 - qBound(0.0, (double)(stops.at(i).first), 1.0);
                b.stop = step + 1 - qBound(0.0, (double)(stops.at(i - 1).first), 1.0);
            ...
            for (int i = 0; i < stops.size() - 1; ++i) {
                QGradientBound b;
                b.start = step + qBound(0.0, (double)(stops.at(i).first), 1.0);
                b.stop = step + qBound(0.0, (double)(stops.at(i + 1).first), 1.0);
...
int QPdfEnginePrivate::generateRadialGradientShader(const QRadialGradient *gradient, const QTransform &matrix, bool alpha)
{
    ...
    s << "<<\n"
        ...
        "/AntiAlias true\n"
        "/Domain [0 1]\n"
        "/Coords [" << (qreal)(p0.x()) << (qreal)(p0.y()) << (qreal)(r0) << (qreal)(p1.x()) << (qreal)(p1.y()) << (qreal)(r1) << "]\n"
        "/Extend [true true]\n"
        "/Function " << function << "0 R\n"
        ">>\n"
        "endobj\n";
